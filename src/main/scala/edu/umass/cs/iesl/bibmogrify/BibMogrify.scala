...
object BibMogrify extends Logging {
  ...
  private val tokensBinary = List("xform" -> ("transforms (" + pm.transformers.keys.mkString(", ") + ")"), "sink" -> ("sinks (" + pm.sinks.keys.mkString(", ") + ")")) //, "input" -> "a text file, or - for stdin")
...
class BibMogrify extends Logging {
  ...
  def run(cl: CommandLine) {
    ...
    val transforms = cl.get("--xform").get.split(",").map(name => BibMogrify.pm.transformers.getOrElse(name, throw new BibMogrifyException(name + " not found")))
