...
object OneLineWriter extends CitationStreamWriter
  ...
  def apply(cms: Seq[CitationMention], out: BufferedWriter) =
    ...
    for (cm <- cms)
      {
      val line = cm.locations.headOption.getOrElse("") + "\t" + cm.primaryId  + "\t" + cm.year  + "\t" + cm.authorFullNames.mkString(", ") + "\t" + cm.title + "\t" + cm.cleanAbstract + "\n"
...
object MalletAbstractWriter extends CitationStreamWriter
  ...
  def apply(cms: Seq[CitationMention], out: BufferedWriter) =
    ...
    for (cm <- cms)
      {
      val line = cm.primaryId + "\t" + cm.year + "\t" + cm.cleanTitleAndAbstract + "\n"
...
object MalletFullWriter extends CitationStreamWriter
  ...
  def apply(cms: Seq[CitationMention], out: BufferedWriter) =
    ...
    for (cm <- cms)
      {
      val line = cm.primaryId + "\t" + cm.year + "\t" + cm.cleanTotal + "\n"
      out.write(line)
      }
    }
  }
