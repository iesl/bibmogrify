...
object VfsToInputStreams extends Transformer[String, NamedInputStream] with NamedPlugin with Logging {
  ...
  private val fsManager: DefaultFileSystemManager = {
    ...
    m.setFilesCache(new LRUFilesCache()) // if a directory has more than 100 entries (or n, if we set it ourselves) this fails; but we don't want to cache all that
    m.init()
    m
  }
  ...
  def getAllInputStreams(fileObject: FileObject): TraversableOnce[NamedInputStream] = {
    ...
    if (fileObject.getType == FileType.FOLDER) {
      ...
      // perhaps that helps conserve memory, but still fileObject.getChildren returns an array and so must read an entire tar or zip file before returning anything.
    }
...
object ArchiveToInputStreams extends Transformer[URL, NamedInputStream] with NamedPlugin with Logging {
  ...
  private  def processTarStream(baseName: String, baseStream: InputStream): Iterator[NamedInputStream] {var nextEntry: Option[NamedInputStream]; def hasNext: Boolean; def readNext: Option[NamedInputStream]; def next(): NamedInputStream} = {
