...
object Pump extends Logging {
  def apply[T](source: TraversableOnce[T], sink: Sink[T]) {
    //val x = source.toIterable.par //toSeq // no clue why toSeq is needed here, but otherwise the map below doesn't work ??  BAD memory use.  even toIterable makes a Stream.
    //logger.warn("Pumping...")
